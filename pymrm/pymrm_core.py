...
def construct_grad(shape, face_positions, cell_centers=None, boundary_conditions=(None, None), axis=0):
    ...
    if isinstance(shape, int):
        ...
        shape = tuple(shape)
        shape_f = list(shape)
    face_positions, cell_centers = generate_grid(
        shape[axis], face_positions, cell_centers, generate_x_c=True)
    ...
    if (boundary_conditions is None):
        ...
        grad_matrix_bc, grad_bc = construct_grad_bc(
            shape, face_positions, cell_centers, boundary_conditions, axis)
...
def construct_grad_bc(shape, face_positions, cell_centers=None, boundary_conditions=(None, None), axis=0):
    ...
    if (shape_t[1] == 1):
        ...
        alpha_1 = (face_positions[1] - face_positions[0]) / (
            (cell_centers[0] - face_positions[0]) * (face_positions[1] - cell_centers[0]))
        alpha_2L = (cell_centers[0] - face_positions[0]) / (
            (face_positions[1] - face_positions[0]) * (face_positions[1] - cell_centers[0]))
        alpha_0L = alpha_1 - alpha_2L
        alpha_2R = -(cell_centers[0] - face_positions[1]) / (
            (face_positions[0] - face_positions[1]) * (face_positions[0] - cell_centers[0]))
        ...
        value = alpha_1 * \
            b[0] * (a[1] * (alpha_0R - alpha_2L) + b[1]) * \
            fctr + np.zeros(shape)
        values[:, 0, :] = np.reshape(value, shape_bc_d)
        value = alpha_1 * \
            b[1] * (a[0] * (-alpha_0L + alpha_2R) - b[0]) * \
            fctr + np.zeros(shape)
        ...
        if (cell_centers is None):
            cell_centers = 0.5*np.array([face_positions[0] + face_positions[1], face_positions[1] + face_positions[2],
                                        face_positions[-3] + face_positions[-2], face_positions[-2] + face_positions[-1]])

        # Get a, b, and d for left bc from dictionary
        a, b, d = unwrap_bc(shape, boundary_conditions[0])
        alpha_1 = (cell_centers[1] - face_positions[0]) / (
            (cell_centers[0] - face_positions[0]) * (cell_centers[1] - cell_centers[0]))
        alpha_2 = (cell_centers[0] - face_positions[0]) / (
            (cell_centers[1] - face_positions[0]) * (cell_centers[1] - cell_centers[0]))
        ...
        alpha_1 = -(cell_centers[-2] - face_positions[-1]) / (
            (cell_centers[-1] - face_positions[-1]) * (cell_centers[-2] - cell_centers[-1]))
        alpha_2 = -(cell_centers[-1] - face_positions[-1]) / (
            (cell_centers[-2] - face_positions[-1]) * (cell_centers[-2] - cell_centers[-1]))
        alpha_0 = alpha_1 - alpha_2
        b = b / alpha_0
        fctr = (a + b)
        np.divide(1, fctr, out=fctr, where=(fctr != 0))
...
def construct_convflux_upwind(shape, face_positions, cell_centers=None, boundary_conditions=(None, None), velocity=1.0, axis=0):
    ...
    if (boundary_conditions == None or boundary_conditions == (None, None)):
        ...
        conv_matrix_bc, conv_bc = construct_convflux_upwind_bc(
            shape, face_positions, cell_centers, boundary_conditions, velocity, axis)
...
def newton(function, initial_guess, args=(), tolerance=1.49012e-08, max_iterations=100, solver=None, callback=None):
    ...
    if (solver == 'spsolve'):
        ...
        def linsolver(Jac, g):
            ...
            Jac_iLU = linalg.spilu(Jac)
            M = linalg.LinearOperator((n, n), Jac_iLU.solve)
            dx_neg, info = linalg.cg(Jac, g, np.zeros(
        ...
        def linsolver(Jac, g):
            ...
            dx_neg, info = linalg.bicgstab(
                Jac, g, np.zeros(n), tol=1e-9, maxiter=10, M=M)
            if info != 0:
    ...
    if (~converged):
        message = f"Newton stopped after {
            it} iterations with max. norm {defect}."
    else:
...
def interp_stagg_to_cntr(staggered_values, face_positions, cell_centers=None, axis=0):
    ...
    if (cell_centers is None):
        ...
        wght = (cell_centers - face_positions[:-1]) / \
            (face_positions[1:] - face_positions[:-1])
...
def interp_cntr_to_stagg(cell_centered_values, face_positions, cell_centers=None, axis=0):
    ...
    if (cell_centers is None):
        cell_centers = 0.5*(face_positions[:-1]+face_positions[1:])
    wght = (face_positions[1:-1] - cell_centers[:-1]) / \
    ...
    if (shape_t[1] == 1):
        ...
        staggered_values[:, 0, :] = (cell_centered_values[:, 0, :]*(cell_centers[1]-face_positions[0]) -
                                     cell_centered_values[:, 1, :]*(cell_centers[0]-face_positions[0]))/(cell_centers[1]-cell_centers[0])
        staggered_values[:, -1, :] = (cell_centered_values[:, -1, :]*(face_positions[-1]-cell_centers[-2]) -
                                      cell_centered_values[:, -2, :]*(face_positions[-1]-cell_centers[-1]))/(cell_centers[-1]-cell_centers[-2])
...
def interp_cntr_to_stagg_tvd(cell_centered_values, face_positions, cell_centers=None, boundary_conditions=None, velocity=0, tvd_limiter=None, axis=0):
    ...
    if (shape_t[1] == 1):
        ...
        fctr_m = ((a[1] * alpha_0R + b[1]) * d[0] -
                  alpha_2L * a[0] * d[1]) * fctr
        fctr_m = fctr_m + np.zeros(shape_bc)
        fctr_m = np.reshape(fctr_m, shape_bc_d)
        staggered_values[:, 0, :] += fctr_m
        fctr_m = ((a[0] * alpha_0L + b[0]) * d[1] -
                  alpha_2R * a[1] * d[0]) * fctr
        fctr_m = fctr_m + np.zeros(shape_bc)
        fctr_m = np.reshape(fctr_m, shape_bc_d)
        ...
        x_C = fltr_v_pos[:, 1:-1, :]*cell_centers[:, :-1, :] + \
            ~fltr_v_pos[:, 1:-1, :]*cell_centers[:, 1:, :]
        x_U = fltr_v_pos[:, 1:-1, :]*np.concatenate((face_positions[:, 0:1, :], cell_centers[:, 0:-2, :]), axis=1) + \
            ~fltr_v_pos[:, 1:-1, :]*np.concatenate(
                (cell_centers[:, 2:, :], face_positions[:, -1:, :]), axis=1)
        x_D = fltr_v_pos[:, 1:-1, :]*cell_centers[:, 1:, :] + \
            ~fltr_v_pos[:, 1:-1, :]*cell_centers[:, :-1, :]
...
def upwind(normalized_concentration_center, normalized_position_center, normalized_position_downwind):
    ...
    normalized_concentration_diff = np.zeros_like(
        normalized_concentration_center)
...
def minmod(normalized_concentration_center, normalized_position_center, normalized_position_downwind):
    ...
    """
    normalized_concentration_diff = np.maximum(0, (normalized_position_downwind-normalized_position_center)*np.minimum(
        normalized_concentration_center/normalized_position_center, (1-normalized_concentration_center)/(1-normalized_position_center)))
    return normalized_concentration_diff
...
def osher(normalized_concentration_center, normalized_position_center, normalized_position_downwind):
    ...
    Returns:
        ndarray: Normalized concentration difference c_norm_d-c_norm_C.
    """
    normalized_concentration_diff = np.maximum(0, np.where(normalized_concentration_center < normalized_position_center/normalized_position_downwind,
                                               (normalized_position_downwind/normalized_position_center - 1)*normalized_concentration_center, 1 - normalized_concentration_center))
    return normalized_concentration_diff
...
def vanleer(normalized_concentration_center, normalized_position_center, normalized_position_downwind):
    ...
    Returns:
        ndarray: Normalized concentration difference c_norm_d-c_norm_C.
    """
    normalized_concentration_diff = np.maximum(0, normalized_concentration_center*(1-normalized_concentration_center)*(
        normalized_position_downwind-normalized_position_center)/(normalized_position_center*(1-normalized_position_center)))
    return normalized_concentration_diff
...
def non_uniform_grid(left_bound, right_bound, num_points, dx_inf, factor):
    ...
    face_positions = left_bound + unif * dx_inf + \
...
def generate_grid(size, face_positions, cell_centers=None, generate_x_c=False):
    if (size+1 == len(face_positions)):
        ...
        if (size+1 == len(face_positions)):
            ...
            face_positions = np.linspace(
                face_positions[0], face_positions[1], size+1)
        elif (face_positions is None or len(face_positions) == 0):
